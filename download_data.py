# Credit: https://github.com/microsoft/CameraTraps/blob/main/data_management/lila/download_lila_subset.py
import json
...
datasets_of_interest = ['Wellington Camera Traps']

species_of_interest = ['bird', 'cat', 'empty', 'hedgehog', 'goat', 'possum',
       'mouse', 'dog', 'pig', 'rabit', 'deer', 'rat', 'unclassifiable',
       'ship rat', 'norway rat', 'mustelid', 'hare']

# Path to write images, metadata downloads, and temporary files 
lila_local_base = os.path.expanduser('~/Project')
...
# Approach 1 - loops over files and downloads images directly in Python
use_azcopy_for_download = False

overwrite_files = False

# Number of concurrent download threads (when not using AzCopy) (AzCopy does its own parallelism)
...
#%% List of files we're going to download 
...
if use_azcopy_for_download:
    ...
    # ds_name = 'Wellington Camera Traps'
